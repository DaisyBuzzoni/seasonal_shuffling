---
title: "Buzzoni D., Cunning R. and Baker A. C. (2022) Initial Algal Symbiont Communities Affect Shuffling to Durusdinium after Coral Bleaching and Reflect Seasonal Differences in Bleaching Sensitivity"
output:
...
ggplot()+
  geom_point(data=sh_emmcontrasts, aes(x=Species,y=(estimate), colour=Species), size=3)+ #large points are showing estimated october:april 
  geom_point(data=Apr_Oct_sh_change, aes(x=Species,y=log10(octtoapr), colour=Species), size=1.2,  position=position_jitter(width=0.1))+ #small points are showing october s:h / april s:h
...
coord_cartesian(clip='on', ylim=c(0,1.0), expand=T)
...
controls<-speciesshuff[c(1,2,5),]
controls$Timepoint<-rep('3',3)
aucall<-rbind(aucall,controls)
...
ggplot(aucall, aes(x = Timepoint, y = fit, group = Species)) +
  ...
  scale_x_discrete(labels=c('Post heat stress','1 month recovery','2 month recovery'))+
  theme_minimal(base_size = 13)%+replace%
   theme(panel.border = element_rect(colour='grey20',fill=NA),
          panel.grid.minor.x = element_blank(),
         panel.grid.major.x = element_blank(),
  ...
  labs(y='Symbiont Shuffling',x='')+
  annotate(geom='text',x=2.2,y=0.23,label=gaindd,size=3.3)+
   annotate(geom='text',x=2.2,y=-0.23,label=lostd,size=3.3)+
...
shufflegroups$Colony<-as.factor(shufflegroups$Colony)
...
shufflegroups$Timepoint<-as.factor(shufflegroups$Timepoint)
