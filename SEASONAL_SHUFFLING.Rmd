...
summary(shuffmod4)
anova(shuffmod4,test='F')# prop d significantly higher after mcav switching compared to ofav/ssid switching p=6.40e-07***
...
shufflegroups$Species<-as.factor(shufflegroups$Species)
shufflegroups$Timepoint<-as.numeric(shufflegroups$Timepoint)
ofavssidpropd<- glm(PropD~Timepoint*initiald*Species,data=filter(shufflegroups,shufflegroups$Species!='M.cavernosa'), family='quasibinomial')
summary(ofavssidpropd)
anova(ofavssidpropd,test='F')
...
###UPDATE FOR ROSS: Due to small sample size, ofav and ssid are grouped together here (model finds no significant effect of species). Now this plot includes all corals. 
